...
class ChooserTexte {
  ...
  **/
  async findTextEntry(){
    ...
    if (commentsEntry) {this.commentsEntry = await commentsEntry.getFile()}
    else { // Le fichier n'existe pas, on le créé
      this.commentsEntry = await this.createCommentsFile(texteEntry)
    }
    console.log("this.commentsEntry", this.commentsEntry)
  ...
  async createCommentsFile(texteEntry){
    const fname = `${texteEntry.affixe}.doctagger`
    console.log("fname:", fname)

    // TODO METTRE : window.PERSISTENT
    window.requestFileSystem(window.TEMPORARY, 1024*1024, function(fs){
      console.log("fs:", fs)
      fs.root.getFile(fname, {create:true}, entry => {
        console.log("entry:", entry)
      }, error)
    }, error)

    // ÇA NE MARCHE PAS NON PLUS AVEC :
    // console.log("-> createCommentsFile")
    // const sandboxed_dir = await getSandboxedFileSystem();
    // console.log("sandboxed_dir:", sandboxed_dir)
    ...
    //     console.log("entrée:", entry)
    // };
    //
    // const new_file = await sandboxed_dir.getFile(fname, {create: true});
    ...
    // }
    // await writer.write(JSON.stringify(alldata));
    // await writer.close();
    // console.log("Data save in .doctagger")

    // const my = this
    // console.log(`-> createCommentsFile(${JSON.stringify(texteEntry)})`)
    // var fname = `${texteEntry.affixe}.doctagger`
    // console.log("fname:", fname)
    // console.log("this.folder", this.folder)
    // const new_file = await this.folder.getFile(fname, {create: true});// ne fonctionne pas
    ...
    // await writer.write(new Blob([JSON.stringify(alldata)]));
    // await writer.close();
    // return new_file
  }
  /** ---------------------------------------------------------------------
